...
- **`aria2.changeOption`** — `select-file` and `bt-remove-unselected-file` are settable (only `pause`, `piece-length`, etc. are excluded). aria2 manages the restart caused by changing `select-file` on an active task; the filter submits options and confirms them through later `tellStatus`/`getOption` readback without forcing a pause. Automatic filtering works on the paused payload child (`pause-metadata=true`). An `OK` callback is request acceptance, not proof that `files[].selected` converged.
...
Automatic `applying-filter` and `restoring-full` also require a one-second stable status/options readback; an OK callback only releases the tick. Keep original and target indexes immutable after the first mutation, including InfoHash reuse. Persist mutation timing and restore any pause ownership from an older version across reloads, but establish a fresh stability window after reload. New option changes do not create an owned pause. Preserve an observed user pause on reused tasks; new-task Download Now/Download Later intent still applies.

Bulk filtering uses a separate persisted checkpoint:

```text
inspecting -> applying -> stable convergence -> filtered
                     \-> bounded retry -> restoring -> failed
legacy pausing/resuming checkpoints -> readback or owned resume -> applying/restoring
```

`applying`/`restoring` callbacks only release the coordinator tick. A later `tellStatus` plus `getOption` must match the target or original selection for a one-second stability window. Bulk `verifiedAt` is reset on reload and on backward wall-clock jumps; persisted observations cannot prove continuous stability while the page was closed. Never count the item filtered until the final stable readback. `pauseOwned` is retained for older checkpoints only: resume an earlier coordinator-owned pause, but preserve an explicit user pause even when filtering succeeds.
...
- **A connected WebSocket does not guarantee an RPC reply.** Every pending request, including one waiting for open, has a 20-second deadline. Success, error, close, and timeout remove the request and cancel its timer before callbacks. Late replies are ignored; expired writes must not be replayed. The filter receives a normal failure callback and reconciles server state on its next turn instead of keeping the single-flight lock forever.
- **Bulk work must not starve automatic work.** When both queues have work, alternate one automatic RPC chain with one bulk RPC chain while retaining the one-chain-in-flight guard. Pinning a bulk GID must not prevent new-task filtering while waiting for option convergence.
- **Only resume pauses owned by the filter.** New active-task mutations do not call `forcePause`. Older persisted `pauseOwned` checkpoints still need recovery; a task paused without this ownership is user-paused. Stable target selection may still count as filtered, but a user-paused task stays paused. Restoration also leaves a user-paused task paused.
